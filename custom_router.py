...
class EnhancedAPIRouter(DefaultRouter):
    ...
    def register(
        self,
        prefix: str,
        viewset_or_router: type[ViewSetMixin] | SimpleRouter | DefaultRouter | Any,
    ...
    def register_viewset(
        self, prefix: str, viewset: type[ViewSetMixin], basename: str | None = None
    ) -> None:
        """
    ...
    def register_router(
        ...
        if namespace is None:
            namespace = (
                prefix.replace("/", "_")
                if self.auto_basename and prefix
                else prefix or "nested"
            )
    ...
    def _is_router(self, obj: Any) -> bool:
        ...
        return (
            ...
            and not self._is_viewset_class(obj)
        )
    ...
    def _is_viewset_class(self, obj: Any) -> bool:
        ...
        return isinstance(obj, type) and (
            issubclass(obj, APIView) or issubclass(obj, ViewSetMixin)
        )
    ...
    def get_urls(self) -> list[Any]:
        ...
        for prefix, viewset_or_router, basename in self.registry:
            if self._is_router(viewset_or_router):
                # Обрабатываем роутер
                router_urls = self._get_router_urls(prefix, viewset_or_router, basename)
                ret.extend(router_urls)
            elif self._is_viewset(viewset_or_router):
                # Обрабатываем ViewSet
                viewset_urls = self._get_viewset_urls(
                    prefix, viewset_or_router, basename
                )
                ret.extend(viewset_urls)
            else:
                # Неизвестный тип - выводим предупреждение
                self._handle_unknown_type(prefix, viewset_or_router, basename, ret)
    ...
    def _handle_unknown_type(
        self, prefix: str, obj: Any, basename: str, ret: list[Any]
    ) -> None:
        """
    ...
    def get_api_root_view(self, api_urls: list[Any] | None = None) -> type[APIView]:
        """
    ...
    **extra: Any,
    ...
    try:
        ...
        if request and hasattr(request, "_request"):
            ...
            if (
                resolver_match
                and hasattr(resolver_match, "namespace")
                and resolver_match.namespace
            ):
                namespaced_viewname = f"{resolver_match.namespace}:{viewname}"
                try:
                    url = django_reverse(
                        namespaced_viewname, args=args, kwargs=kwargs, **extra
                    )
